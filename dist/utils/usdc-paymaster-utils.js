 ...
 */
export async function getUsdcQuote(pimlicoClient, usdcAddress) {
 ...
 */
export async function approveUsdcForPaymaster(smartAccountClient, usdcAddress, paymasterAddress, amountInEth) {
 ...
 * @param usdcAddress The USDC token address on the current network
 */
export async function getUsdcPaymasterData(pimlicoClient, userOperation, // Type will be more specific with permissionless.js v1.0.0+
usdcAddress) {
    // With permissionless.js v1.0.0+, this would be something like:
    /*
 ...
 */
export async function sendTransactionWithUsdcGas(smartAccountClient, pimlicoClient, params) {
    const { to, value, data = '0x', usdcAddress, approvalAmount = '1' } = params;
