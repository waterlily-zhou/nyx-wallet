...
router.get('/api/check-recipient', async (ctx) => {
    try {
        ...
        const isNew = !savedAddresses.some(saved => saved.address.toLowerCase() === address.toLowerCase());
...
router.post('/api/get-calldata', async (ctx) => {
    try {
        const { fromAddress, toAddress, amount = '0', currency = 'ETH', message = '', nonce = '0', gasPaymentMethod = 'default', submissionMethod = 'direct' } = ctx.request.body;
        ...
        const messageHex = message ? '0x' + Buffer.from(message).toString('hex') : '0x';
        ...
        try {
            ...
            const executeCalldata = encodeFunctionData({
                ...
                args: [
                    currency === 'USDC' ? '0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238' : toAddress,
                    valueToSend,
                    txData
                ]
            });
...
router.post('/api/send-transaction', async (ctx) => {
    try {
        const { recipient, message = '', amount = '0', currency = 'ETH', gasPaymentMethod = 'default', submissionMethod = 'direct' } = ctx.request.body;
        if (!recipient) {
            ctx.status = 400;
